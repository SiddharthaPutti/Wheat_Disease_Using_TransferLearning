...
def train(config: DictConfig):
    ...
    trainloader = DataLoader(dataset=trainset, batch_size=config.training.batch_size, shuffle=True, num_workers=cpu_count() - 2)
    testloader = DataLoader(dataset=testset, batch_size=config.training.batch_size, shuffle=True, num_workers=cpu_count() - 2)

    print('done')
    if config.models.model == 'vgg19':
        model = torchvision.models.vgg19(pretrained=config.pretrain)
